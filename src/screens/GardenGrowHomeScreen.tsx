...
type Props = { navigation: ScreenNavigationProp<'GardenGrowHome'> };

export const GardenGrowHomeScreen: React.FC<Props> = ({ navigation }) => {
  const { t } = useTranslation();
  const { bestScore } = useGardenGrow();
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.title}>{t('gardenGrow.home.title')}</Text>
        <Text style={styles.subtitle}>{t('gardenGrow.home.subtitle')}</Text>
        {bestScore > 0 && (<View style={styles.bestScoreCard}><Text style={styles.bestScoreLabel}>{t('gardenGrow.home.bestScore')}</Text><Text style={styles.bestScoreValue}>{bestScore}</Text></View>)}
        <TouchableOpacity style={styles.playButton} onPress={() => navigation.navigate('GardenGrowGame')} activeOpacity={0.85}><Text style={styles.playButtonText}>{t('gardenGrow.home.play')}</Text></TouchableOpacity>
        <Text style={styles.instructions}>{t('gardenGrow.home.instructions')}</Text>
      </View>
    </SafeAreaView>
  );
};
