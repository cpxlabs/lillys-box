  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.title}>{t('balloonFloat.home.title')}</Text>
        <Text style={styles.subtitle}>{t('balloonFloat.home.subtitle')}</Text>
        {bestScore > 0 && (<View style={styles.bestScoreCard}><Text style={styles.bestScoreLabel}>{t('balloonFloat.home.bestScore')}</Text><Text style={styles.bestScoreValue}>{bestScore}</Text></View>)}
        <TouchableOpacity style={styles.playButton} onPress={() => navigation.navigate('BalloonFloatGame')} activeOpacity={0.85}><Text style={styles.playButtonText}>{t('balloonFloat.home.play')}</Text></TouchableOpacity>
        <Text style={styles.instructions}>{t('balloonFloat.home.instructions')}</Text>
      </View>
    </SafeAreaView>
  );
};
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 36, fontWeight: '800', color: '#2196f3', marginBottom: 8 },
  ...
  bestScoreValue: { fontSize: 32, fontWeight: '800', color: '#2196f3' },
  playButton: { backgroundColor: '#2196f3', paddingVertical: 18, paddingHorizontal: 52, borderRadius: 32, shadowColor: '#2196f3', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 5 },
