...
export const BubblePopHomeScreen: React.FC<Props> = ({ navigation }) => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.title}>{t('bubblePop.home.title')}</Text>
        <Text style={styles.subtitle}>{t('bubblePop.home.subtitle')}</Text>
        {bestScore > 0 && (
          <View style={styles.bestScoreCard}>
            <Text style={styles.bestScoreLabel}>{t('bubblePop.home.bestScore')}</Text>
            <Text style={styles.bestScoreValue}>{bestScore}</Text>
          </View>
        )}
        <TouchableOpacity style={styles.playButton} onPress={() => navigation.navigate('BubblePopGame')} activeOpacity={0.85}>
          <Text style={styles.playButtonText}>{t('bubblePop.home.play')}</Text>
        </TouchableOpacity>
        <Text style={styles.instructions}>{t('bubblePop.home.instructions')}</Text>
      </View>
    </SafeAreaView>
  );
};
...
const styles = StyleSheet.create({
  ...
  bestScoreValue: { fontSize: 32, fontWeight: '800', color: '#4a90d9' },
  playButton: { backgroundColor: '#4a90d9', paddingVertical: 18, paddingHorizontal: 52, borderRadius: 32, shadowColor: '#4a90d9', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 5 },
