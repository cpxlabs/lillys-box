...
type Props = { navigation: ScreenNavigationProp<'HideAndSeekHome'> };

export const HideAndSeekHomeScreen: React.FC<Props> = ({ navigation }) => {
  const { t } = useTranslation();
  const { bestScore } = useHideAndSeek();
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.title}>{t('hideAndSeek.home.title')}</Text>
        <Text style={styles.subtitle}>{t('hideAndSeek.home.subtitle')}</Text>
        {bestScore > 0 && (<View style={styles.bestScoreCard}><Text style={styles.bestScoreLabel}>{t('hideAndSeek.home.bestScore')}</Text><Text style={styles.bestScoreValue}>{bestScore}</Text></View>)}
        <TouchableOpacity style={styles.playButton} onPress={() => navigation.navigate('HideAndSeekGame')} activeOpacity={0.85}><Text style={styles.playButtonText}>{t('hideAndSeek.home.play')}</Text></TouchableOpacity>
        <Text style={styles.instructions}>{t('hideAndSeek.home.instructions')}</Text>
      </View>
    </SafeAreaView>
  );
};
